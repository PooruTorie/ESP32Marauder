...
void EvilPortal::setupServer() {
    ...
    secServer.on("/", HTTP_GET, [this](AsyncWebServerRequest *request) {
        AsyncWebServerResponse *response = request->beginResponse(300);
        String host = request->getHeader("Host")->value();
        response->addHeader("Location", "http://" + host);
        request->send(response);
        Serial.println("https redirect");
#ifdef HAS_SCREEN
        this->sendToDisplay("Client redirect https");
#endif
    });
...
#endif
            Serial.println(
                    "The provided HTML is too large. Byte limit is " + (String) MAX_HTML_SIZE + "\nUse stopscan...");
            return false;
        }
        String html = "";
...
#endif
            Serial.println(
                    "The given AP name is too large. Byte limit is " + (String) MAX_AP_NAME_SIZE + "\nUse stopscan...");
        } else {
